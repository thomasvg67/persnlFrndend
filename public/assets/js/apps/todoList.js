...
function dynamicBadgeNotification( setTodoCategoryCount ) {
  ...
  var get_TodoPendingListParentsDiv = $('.todo-item.todo-task-pending');
  ...
  var get_PendingListElementsCount = get_TodoPendingListParentsDiv.length;
  ...
  var getBadgePendingTaskListDiv = $('#todo-task-pending .todo-badge');
  ...
  }else if (todoCategoryCount === 'allListpending') {
    if (get_PendingListElementsCount === 0) {
      getBadgePendingTaskListDiv.text('');
      return;
    }
    if (get_PendingListElementsCount > 9) {
        getBadgePendingTaskListDiv.css({
            padding: '2px 0px',
            height: '25px',
            width: '25px'
        });
    } else if (get_PendingListElementsCount <= 9) {
        getBadgePendingTaskListDiv.removeAttr('style');
    }
    getBadgePendingTaskListDiv.text(get_PendingListElementsCount);
  }
}
...
setTimeout(function(){
  ...
  new dynamicBadgeNotification('allListpending');
...
// var quill = new Quill('#taskdescription1', {
//   modules: {
//     toolbar: [
//       [{ header: [1, 2, false] }],
...
function checkCheckbox() {
  $('body').on('click','.todo-item input[type="checkbox"]',function() {
    if ($(this).is(":checked")) {
...
function importantDropdown() {
  $('body').on('click','.important', function() {
...
function priorityDropdown() {
  $('body').on('click','.priority-dropdown .dropdown-menu .dropdown-item', function(event) {
...
function editDropdown() {
  $('body').on('click','.action-dropdown .dropdown-menu .edit.dropdown-item',function(event) {
...
function todoItem() {
  $('body').on('click','.todo-item .todo-content', function(event) {
    event.preventDefault();
