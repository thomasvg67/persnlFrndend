...
const MutualFunds = () => {
  ...
  const allowedTypes = ["follow", "snoozed", "wishlist", "other"];
  ...
  useEffect(() => {
    setActiveType(allowedTypes.includes(type) ? type : "");
  }, [type]);
  ...
  useEffect(() => {
    ...
    // Set filter dropdown
    const $filter = $('#medicine-filter');
    if ($filter.length) {
      $filter.val(activeType).trigger('change');
    }

    // Reload table with new type
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        tableRef.current = $('#style-1').DataTable({
          processing: false,
          serverSide: true,
          order: [[8, 'desc']],
          columnDefs: [
          ...
          buttons: [
            ...
            },
            {
              ...
              action: async function () {
                if (selectedRowIds.current.size === 0) {
                  // alert("No contacts selected");
                  return;
                }

                for (const id of selectedRowIds.current) {
        ...
        if (activeType) {
          setTimeout(() => {
            $('#medicine-filter').val(activeType).trigger('change');
          }, 0);
        }

        $('#medicine-filter').on('change', function () {
        ...
        $(document).on("change", ".chk-parent", function () {
          ...
          $("#style-1 tbody .child-chk").each(function () {
            ...
            if (checked) {
              selectedRowIds.current.add(id);
            } else {
              selectedRowIds.current.delete(id);
            }
          });
        });
        ...
        $('.customToggleBtns').html(`
          <div class="toggle-list d-inline-block">
            ...
            <a class="btn btn-sm btn-primary toggle-vis ml-2 mb-2" data-column="6">Type</a>
        ...
        $(document).on('click', 'a.toggle-vis', function (e) {
          e.preventDefault();
          const column = tableRef.current.column($(this).data('column'));
          column.visible(!column.visible());
        });

        if (typeof multiCheck === 'function') multiCheck(dt);
        setLoading(false);
      } catch (err) {
        console.error(err);
        setLoading(false);
