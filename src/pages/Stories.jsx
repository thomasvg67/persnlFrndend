...
const Stories = () => {
  ...
  const filteredStories = names?.filter(n =>
  ...
  useEffect(() => {
    const fetchStories = async () => {
      ...
      try {
        ...
        const res = await axios.get(`${VITE_BASE_URL}/api/stories?page=${currentPage}&category=${selectedCategory}`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        // Make sure response matches new backend structure
        setNames(res.data.stories); // fix here
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editId) {
        res = await axios.put(`${VITE_BASE_URL}/api/stories/${editId}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success('Story updated successfully!');
      } else {
        res = await axios.post(`${VITE_BASE_URL}/api/stories`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      setNames(prev =>
        editId
          ? prev.map(n => (n._id === editId ? res.data.story : n))
          : [res.data.story, ...prev]
      );
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${VITE_BASE_URL}/api/stories/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleExportDoc = () => {
    const html = `
      ...
      <table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">
        <thead>
          <tr>
            <th>Title</th><th>Sub Category</th><th>Description</th><th>Created On</th>
          </tr>
        </thead>
        <tbody>
  ...
  const handleExportExcel = () => {
    const data = filteredStories.map(n => ({
      Title: n.title,
    ...
    XLSX.writeFile(workbook, 'Stories.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    ...
    doc.setFontSize(16);
    doc.text("Stories Report", 105, 15, { align: "center" });
    doc.setLineWidth(0.5);
    doc.line(10, 20, 200, 20);

    const rows = filteredStories.map((n, idx) => [
      idx + 1,
      n.title,
      n.subCategory,
    ...
    autoTable.default(doc, {
      startY: 25,
      head: [['#', 'Title', 'SubCategory', 'Description', 'Created On']],
      body: rows,
      styles: { fontSize: 9 },
  ...
  const handlePrint = () => {
    const html = `
      ...
      <table border="1" style="border-collapse:collapse;width:100%">
        <thead>
          <tr>
            <th>Title</th><th>SubCategory</th><th>Description</th><th>Created On</th>
          </tr>
        </thead>
        <tbody>
          ${filteredStories.map(n => `
                        ...
                        <form className="form-inline my-2 my-lg-0 w-100">
                          <div className="d-flex align-items-center w-100">
                            <div className="d-flex align-items-center flex-grow-1 mr-3">
                              ...
                              <input type="text" className="form-control product-search" id="input-search" placeholder="Search Stories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                            </div>
                                  ...
                                  <div className="add-contact-content">
                                    <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
                                      <div className="row">
                                        ...
                                        {/* Name */}
                                        <div className="col-md-12 mb-3">
                                          <label className={`text-left d-block ${errors.title ? 'text-danger' : ''}`}>
                          ...
                          <div className="item-content">
                            <div className="user-profile">
                              ...
                              <div className="user-meta-info">
                                <p className="user-name" data-name="Alan Green">{n.title}</p>
