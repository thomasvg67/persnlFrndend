...
const Medicines = () => {
  ...
  const [medicine, setMedicine] = useState('Ayurveda');
  ...
  const filteredMedicines = (nameList || []).filter(n =>
    n.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    n.medicine?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      ...
      try {
        ...
        const res = await axios.get(`${VITE_BASE_URL}/api/medicines?page=${currentPage}&category=${selectedMedicine}`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        // Make sure response matches new backend structure
        setNameList(res.data.medicine);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editId) {
        res = await axios.put(`${VITE_BASE_URL}/api/medicines/${editId}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success('Medicine entry updated successfully!');
      } else {
        res = await axios.post(`${VITE_BASE_URL}/api/medicines`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      setNameList(prev => {
        ...
        return editId
          ? prev.map(n => (n._id === editId ? res.data.medicine : n))
          : [res.data.medicine, ...prev];
      });
  ...
  const handleExportDoc = () => {
    const html = `
      ...
      <table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">
        <thead>
          <tr>
            <th>Medicine</th><th>Name</th><th>Description</th><th>Source</th><th>No. of Alerts</th><th>Start From</th><th>Created On</th>
          </tr>
        </thead>
        <tbody>
          ${filteredMedicines.map(n => `
            <tr>
              <td>${n.medicine}</td>
              <td>${n.name}</td>
  ...
  const handleExportExcel = () => {
    const data = filteredMedicines.map(n => ({
      Medicine: n.medicine,
      Name: n.name,
    ...
    XLSX.writeFile(workbook, 'Medicine.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    ...
    doc.setFontSize(16);
    doc.text("Medicine Report", 105, 15, { align: "center" });
    doc.setLineWidth(0.5);
    doc.line(10, 20, 200, 20);

    const rows = filteredMedicines.map((n, idx) => [
      idx + 1,
      n.medicine,
      n.name,
    ...
    autoTable.default(doc, {
      startY: 25,
      head: [['#', 'Medicine', 'Name', 'Description', 'Source', 'Created On', 'No. of Alerts', 'Start From']],
      body: rows,
      styles: { fontSize: 9 },
  ...
  const handlePrint = () => {
    const html = `
      ...
      <table border="1" style="border-collapse:collapse;width:100%">
        <thead>
          <tr>
            <th>Medicine</th><th>Name</th><th>Description</th><th>Source</th><th>Created On</th><th>No. of Alerts</th><th>Start From</th>
          </tr>
        </thead>
        <tbody>
          ${filteredMedicines.map(n => `
                                    ...
                                    <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
                                      <div className="row">
                                        ...
                                        <div className="col-md-12 mb-3">
                                          <label className={`text-left d-block ${errors.medicine ? 'text-danger' : ''}`}>
                                            Medicines <span className="text-danger">*</span>
                                          </label>
                                          <select
                                            className={`form-control ${errors.medicine ? 'border border-danger' : ''}`}
                                            value={medicine}
                                          ...
                                          {errors.medicine && <small className="text-danger">{errors.medicine}</small>}
                                        </div>

                                        {/* Names */}
                                        <div className="col-md-12 mb-3">
                                          <label className={`text-left d-block ${errors.name ? 'text-danger' : ''}`}>
                          ...
                          <div className="item-content">
                            <div className="user-profile">
                              ...
                              <div className="user-meta-info">
                                <p className="user-name" data-name="Alan Green">{n.name}</p>
                            ...
                            <div className="user-email">
                              ...
                              <p
                                ...
                                dangerouslySetInnerHTML={{
                                  __html: (n.description.replace(/<[^>]+>/g, '').slice(0, 10) + '...')
                                }}
                              />
                            </div>
