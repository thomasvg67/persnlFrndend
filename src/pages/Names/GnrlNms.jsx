...
const GnrlNms = () => {
  ...
  useEffect(() => {
    const fetchNames = async () => {
      ...
      try {
        ...
        const res = await axios.get(`${VITE_BASE_URL}/api/names?page=${currentPage}&category=${selectedCategory}`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        // Make sure response matches new backend structure
        setNames(res.data.names); // fix here
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editId) {
        res = await axios.put(`${VITE_BASE_URL}/api/names/${editId}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success('Name updated successfully!');
      } else {
        res = await axios.post(`${VITE_BASE_URL}/api/names`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      setNames(prev =>
        editId
          ? prev.map(n => (n._id === editId ? res.data.name : n))
          : [res.data.name, ...prev]
      );
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${VITE_BASE_URL}/api/names/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleExportDoc = () => {
    const html = `
      ...
      <table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">
        <thead>
          <tr>
            <th>Name</th><th>Sub Category</th><th>Meaning</th><th>Source</th><th>Created On</th>
          </tr>
        </thead>
        <tbody>
          ${filteredNames.map(n => `
  ...
  const handleExportExcel = () => {
    const data = filteredNames.map(n => ({
      Name: n.name,
    ...
    XLSX.writeFile(workbook, 'Names.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    ...
    doc.setFontSize(16);
    doc.text("Names Report", 105, 15, { align: "center" });
    doc.setLineWidth(0.5);
    doc.line(10, 20, 200, 20);

    const rows = filteredNames.map((n, idx) => [
      idx + 1,
      n.name,
      n.subCategory,
      n.meaning.replace(/<[^>]+>/g, '') || '-',
    ...
    autoTable.default(doc, {
      startY: 25,
      head: [['#', 'Name', 'SubCategory', 'Meaning', 'Source', 'Created On']],
      body: rows,
      styles: { fontSize: 9 },
      headStyles: { fillColor: [22, 160, 133] }
    });

    doc.save("names_list.pdf");
  };
  ...
  const handlePrint = () => {
    const html = `
      ...
      <table border="1" style="border-collapse:collapse;width:100%">
        <thead>
          <tr>
            <th>Name</th><th>SubCategory</th><th>Meaning</th><th>Source</th><th>Created On</th>
          </tr>
        </thead>
        <tbody>
          ${filteredNames.map(n => `
                        ...
                        <form className="form-inline my-2 my-lg-0 w-100">
                          <div className="d-flex align-items-center w-100">
                            <div className="d-flex align-items-center flex-grow-1 mr-3">
                              ...
                              <input type="text" className="form-control product-search" id="input-search" placeholder="Search Names..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                            </div>
