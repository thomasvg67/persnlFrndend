...
const SharesFollow = () => {
  ...
  const fetchShareForEdit = async (id) => {
    try {
      const response = await fetch(`${VITE_BASE_URL}/api/shares/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      setForm({
        ...
        bseCode: share.bseCode || '',
        isin: share.isin || '',
        sector: share.sector || '',
        actionType: share.actionType || '',
        followUp: share.followUp || '',
        startFrom: share.startFrom
          ? new Date(share.startFrom).toISOString().slice(0, 10)
          : '',
        description: share.description || '',
        status: share.status ?? true
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingId
        ? `${VITE_BASE_URL}/api/shares/edit/${editingId}`
        : `${VITE_BASE_URL}/api/shares/add`;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const defaultTypeFilter = 'follow';
        ...
        const c1 = $('#style-1').DataTable({
          ...
          ajax: {
            ...
            dataSrc: function (json) {
              ...
              return json.data.map((share, index) => ({
                record: index + 1,
                name: share.name,
                nseCode: share.nseCode,
                bseCode: share.bseCode,
            ...
            },
            {
              ...
              action: async function () {
                ...
                for (const id of selectedRowIds) {
                  await fetch(`${VITE_BASE_URL}/api/shares/delete/${id}`, {
                    method: "DELETE",
  ...
  `;
        ...
        $(document).on("click", ".edit-contact", function (e) {
          ...
          fetchShareForEdit(id).then(success => {
        ...
        $(document).on("click", ".delete-contact", async function (e) {
          ...
          try {
            const response = await fetch(
              `${VITE_BASE_URL}/api/shares/delete/${id}`,
              {
                method: "DELETE",
                headers: {
                  Authorization: `Bearer ${token}`
