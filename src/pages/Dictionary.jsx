...
const Dictionary = () => {
    ...
    const [meaningModalVisible, setMeaningModalVisible] = useState(false);
    const [meaningContent, setMeaningContent] = useState('');

    const filteredWords = wordList?.filter(n =>
        n.words.toLowerCase().includes(searchQuery.toLowerCase()) ||
        n.language.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    useEffect(() => {
        const fetchDictionary = async () => {
            ...
            try {
                ...
                const res = await axios.get(`${VITE_BASE_URL}/api/dictionary?page=${currentPage}&category=${selectedLanguage}`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let res;
            if (editId) {
                res = await axios.put(`${VITE_BASE_URL}/api/dictionary/${editId}`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                toast.success('Dictionary entry updated successfully!');
            } else {
                res = await axios.post(`${VITE_BASE_URL}/api/dictionary`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            ...
            setWordList(prev =>
                editId
                    ? prev.map(n => (n._id === editId ? res.data.words : n))
                    : [res.data.words, ...prev]
            );
        } catch (err) {
            console.error(err);
    ...
    const handleEdit = (data) => {
        ...
        setNoOfAlerts(data.noOfAlerts || 0);
        setStartFrom(data.startFrom || 0);
        setActive(data.sts);
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Are you sure to delete this name?')) return;
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${VITE_BASE_URL}/api/dictionary/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            toast.success('Deleted successfully');
            setWordList(prev => prev.filter(n => n._id !== id));
        } catch (err) {
    ...
    const openMeaningModal = (html) => {
        setMeaningContent(html || '');
        setMeaningModalVisible(true);
    };

    const validate = () => {
        const newErrors = {};
        if (!language) newErrors.language = 'Language is required';
        if (!word.trim()) newErrors.word = 'Words are required';
        setErrors(newErrors);
    ...
    const handleExportDoc = () => {
      ...
      <table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">
        <thead>
          <tr>
            <th>Language</th><th>Words</th><th>Meaning</th><th>Source</th><th>No. of Alerts</th><th>Start From</th><th>Created On</th>
          </tr>
        </thead>
        <tbody>
          ${filteredWords.map(n => `
    ...
    const handleExportPdf = async () => {
        ...
        doc.setFontSize(16);
        doc.text("Dictionary Report", 105, 15, { align: "center" });
        doc.setLineWidth(0.5);
        doc.line(10, 20, 200, 20);

        const rows = filteredWords.map((n, idx) => [
            idx + 1,
            n.language,
            n.words,
            n.meaning.replace(/<[^>]+>/g, '') || '-',
        ...
        autoTable.default(doc, {
            startY: 25,
            head: [['#', 'Language', 'Words', 'Meaning', 'Source', 'Created On', 'No. of Alerts', 'Start From']],
            body: rows,
            styles: { fontSize: 9 },
            headStyles: { fillColor: [22, 160, 133] }
        });

        doc.save("dictionary_report.pdf");
    };
    ...
    const handlePrint = () => {
        const html = `
      <h2>Dictionary Report</h2>
      <table border="1" style="border-collapse:collapse;width:100%">
        <thead>
          <tr>
            <th>Language</th><th>Words</th><th>Meaning</th><th>Source</th><th>Created On</th><th>No. of Alerts</th><th>Start From</th>
          </tr>
        </thead>
        <tbody>
          ${filteredWords.map(n => `
                                                ...
                                                <form className="form-inline my-2 my-lg-0 w-100">
                                                    <div className="d-flex align-items-center w-100">
                                                        <div className="d-flex align-items-center flex-grow-1 mr-3">
                                                            ...
                                                            <input type="text" className="form-control product-search" id="input-search" placeholder="Search word..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                                                        </div>
                                                                        ...
                                                                        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
                                                                            <div className="row">
                                                                                ...
                                                                                <div className="col-md-12 mb-3">
                                                                                    <label className={`text-left d-block ${errors.language ? 'text-danger' : ''}`}>
                                                                                        Language <span className="text-danger">*</span>
                                                                                    </label>
                                                                                    <select
                                                                                        className={`form-control ${errors.language ? 'border border-danger' : ''}`}
                                                                                        value={language}
                                                                                ...
                                                                                {/* Words */}
                                                                                <div className="col-md-12 mb-3">
                                                                                    <label className={`text-left d-block ${errors.word ? 'text-danger' : ''}`}>
                                                                                        Words <span className="text-danger">*</span>
                                                                                    </label>
                                                                                    <input
                                                                                        type="text"
                                                                                        className={`form-control ${errors.word ? 'border border-danger' : ''}`}
                                                                                        value={word}
                                                                                        onChange={(e) => setWord(e.target.value)}
                                                                                        required
                                                                                    />
                                                                                    {errors.word && <small className="text-danger">{errors.word}</small>}
                                                                                </div>

                                                                                {/* Meaning */}
                                                                                <div className="col-md-12 mb-3">
                                                                                    <label className="text-left d-block">Meaning</label>
                                                                                ...
                                                                                {/* No of alerts */}
                                                                                <div className="col-md-12 mb-3">
                                                                                    <label className="text-left d-block">Number of Alerts</label>
                                                                                    <input
                                                                                        type="Number"
                                                                                        className="form-control"
                                                                                        value={noOfAlerts}
                                                                                        onChange={(e) => setNoOfAlerts(e.target.value)}
                                                                                    />
                                                                                </div>

                                                                                {/* Start from */}
                                                                                <div className="col-md-12 mb-3">
                                                                                    <label className="text-left d-block">Start From</label>
                                                                                    <input
                                                                                        type="date"
                                                                                        className="form-control"
                                                                                        value={startFrom}
                                        ...
                                        <div className={`searchable-items ${viewMode}`}>
                                            <div className="items items-header-section">
                                                <div className="item-content">
                                                    ...
                                                    <div className="user-location">
                                                        <h4 style={{ marginLeft: 0 }}>No of Alerts</h4>
                                                    </div>
                                                    <div className="user-location">
                                                        <h4 style={{ marginLeft: 0 }}>Start From</h4>
                                                    </div>
                                                    ...
                                                    <div className="item-content">
                                                        <div className="user-profile">
                                                            ...
                                                            <div className="user-meta-info">
                                                                <p className="user-name" data-name="Alan Green">{n.words}</p>
                                                                <p className="user-work" data-occupation="Web Developer">{n.language}</p>
                                                            </div>
                                                        </div>
                                                        <div className="user-email">
                                                            <p className="info-title">Meaning: </p>
                                                            <p
                                                                className="usr-email-addr"
                                                                style={{ cursor: 'pointer' }}
                                                                onClick={() => openMeaningModal(n.meaning)}
                                                                dangerouslySetInnerHTML={{
                                                                    __html:
                                                                        (n.meaning || '').replace(/<[^>]+>/g, '').slice(0, 10) +
                                                                        ((n.meaning || '').replace(/<[^>]+>/g, '').length > 10 ? '...' : '')
                                                                }}
                                                            />
                                                        </div>
                                                        ...
                                                        <div className="user-location">
                                                            <p className="info-title">No of alerts: </p>
                                                            <p className="usr-location" data-location="Boston, USA">{n.noOfAlerts || '-'}</p>
                                                        </div>
                                                        <div className="user-location">
                                                            <p className="info-title">Start From: </p>
                                                            <p className="usr-location" data-location="Boston, USA">{n.startFrom || '-'}</p>
                                                        </div>
                                                        <div className="user-phone">
                                                ...
                                                <div className="modal-dialog modal-dialog-centered modal-lg" role="document">
                                                    <div className="modal-content">
                                                        <div className="modal-header">
                                                            <h5 className="modal-title">Meaning</h5>
                                                            <button type="button" className="close" onClick={() => setMeaningModalVisible(false)}>
                                                                <span>&times;</span>
                                                            </button>
                                                        </div>
                                                        <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                                                            <div dangerouslySetInnerHTML={{ __html: meaningContent }} />
                                                        </div>
                                                        <div className="modal-footer">
                                                            <button className="btn btn-secondary" onClick={() => setMeaningModalVisible(false)}>Close</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
