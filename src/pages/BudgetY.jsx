...
const BudgetY = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${VITE_BASE_URL}/api/budgetYearly`, {
    ...
    if (!name.trim()) {
        toast.warn('BudgetYearly name is required');
        return;
    }

    setOperationLoading(true);
    try {
        await axios.post(`${VITE_BASE_URL}/api/budgetYearly`, { name }, {
            headers: { Authorization: `Bearer ${token}` },
        });
        toast.success('BudgetYearly added successfully!');
        setName('');
        fetchData();
        $('#addListModal').modal('hide');
    } catch (err) {
        toast.error(err.response?.data?.message || 'Failed to add budgetYearly');
    } finally {
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/budgetYearly/${selectedList._id}`, { name }, {
            headers: { Authorization: `Bearer ${token}` },
        });
        toast.success('BudgetYearly updated successfully!');
        setName('');
        setSelectedList(null);
        fetchData();
        $('#addListModal').modal('hide');
    } catch (err) {
        toast.error(err.response?.data?.message || 'Failed to edit budgetYearly');
    } finally {
        setOperationLoading(false);
    }
};
    ...
    setOperationLoading(true);
    try {
        await axios.post(`${VITE_BASE_URL}/api/budgetYearly/${selectedList._id}/visions`, {
            title: taskTitle,
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/budgetYearly/${selectedList._id}/visions/${selectedTask._id}`, {
            title: taskTitle,
            description: taskDesc,
        }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    try {
        if (deleteType === 'list' && deleteList) {
            await axios.delete(`${VITE_BASE_URL}/api/budgetYearly/${deleteList._id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            toast.success('BudgetYearly deleted successfully');
        } else if (deleteType === 'task' && deleteTask && selectedList) {
            await axios.delete(`${VITE_BASE_URL}/api/budgetYearly/${selectedList._id}/visions/${deleteTask._id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            toast.success('Budget deleted successfully');
        } else if (deleteType === 'clear' && clearTasksList) {
            await axios.put(`${VITE_BASE_URL}/api/budgetYearly/${clearTasksList._id}/clear-visions`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    if (!window.confirm('Are you sure you want to delete all Budgets in this budgetYearly?')) return;

    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/budgetYearly/${listId}/clear-visions`, {}, {
            headers: { Authorization: `Bearer ${token}` },
        });
                ...
                <div id="content" className="main-content">
                    <div className="layout-px-spacing">
                        <div className="action-btn layout-top-spacing mb-5">
                            <button id="add-list" className="btn btn-dark" onClick={onAddList}>Add Budget Yearly</button>
                        </div>
                            ...
                            <div className="modal-dialog modal-dialog-centered modal-xl" role="document">
                                <div className="modal-content">
                                    <div className="modal-body">
                                        <div className="compose-box">
                                            <div className="compose-content" id="addTaskModalTitle">
                                                <h5 className="add-task-title" style={{ display: selectedTask ? 'none' : 'block' }}>Add Budget Yearly</h5>
                                                <h5 className="edit-task-title" style={{ display: selectedTask ? 'block' : 'none' }}>Edit Budget Yearly</h5>
                                    ...
                                    <div className="modal-footer">
                                        ...
                                        {!selectedTask && (<button data-btnfn="addTask" className="btn add-tsk" onClick={handleAddTask}>Add Budget Yearly</button>)}
                                    ...
                                    <div className="modal-body">
                                        <div className="compose-box">
                                            <div className="compose-content" id="addListModalTitle">
                                                <h5 className="add-list-title" style={{ display: selectedList ? 'none' : 'block' }}>Add Budget Yearly</h5>
                                                <h5 className="edit-list-title" style={{ display: selectedList ? 'block' : 'none' }}>Edit Budget Yearly</h5>
                                                <form action="javascript:void(0);" onSubmit={(e) => e.preventDefault()}>
                                                    <div className="row">
                                                        <div className="col-md-12">
                                                            <div className="list-title">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="feather feather-list"><line x1={8} y1={6} x2={21} y2={6} /><line x1={8} y1={12} x2={21} y2={12} /><line x1={8} y1={18} x2={21} y2={18} /><line x1={3} y1={6} x2={3} y2={6} /><line x1={3} y1={12} x2={3} y2={12} /><line x1={3} y1={18} x2={3} y2={18} /></svg>
                                                                <input id="s-list-name" type="text" placeholder="Budget Yearly Name" className="form-control" name="task" value={name} onChange={(e) => setName(e.target.value)} required />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </form>
                            ...
                            <div className="modal-dialog modal-dialog-centered" role="document">
                                <div className="modal-content" id="deleteConformationLabel">
                                    <div className="modal-header">
                                        ...
                                        <h5 className="modal-title" id="exampleModalLabel">Delete the BudgetYearly?</h5>
                                        <button type="button" className="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">×</span>
                                        </button>
                                    </div>
                                    <div className="modal-body">
                                        <p className=""> {deleteType === 'list' && "If you delete the BudgetYearly it will be gone forever. Are you sure you want to proceed?"}</p>
                                        <p className=""> {deleteType === 'task' && "If you delete the BudgetYearly it will be gone forever. Are you sure you want to proceed?"}</p>
                                        <p className="">{deleteType === 'clear' && "If you clear all the BudgetYearly it will be gone forever. Are you sure you want to proceed?"}</p>
                                    </div>
                                    <div className="modal-footer">
