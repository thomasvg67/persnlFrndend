...
const EditUser = () => {
    ...
    const { token,user, setProfile } = useContext(AuthContext);
...
const isLoggedInUserAdmin = user?.role === 'adm';
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${VITE_BASE_URL}/api/users/${uId}`, {
                    headers: {
    ...
    const handleSaveAll = async () => {
        ...
        try {
            ...
            formData.append('workExp', JSON.stringify(filteredWorkExp));
            ...
            await axios.put(`${VITE_BASE_URL}/api/users/${uId}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            const updatedProfile = await axios.get(`${VITE_BASE_URL}/api/users/${uId}`, {
                                    ...
                                    <div className="scrollspy-example" data-spy="scroll" data-target="#account-settings-scroll" data-offset={-100}>
                                        <div className="row">
                                            <GeneralInfo name={name} setName={setName}
                                                ...
                                                role={role} setRole={setRole}  // 👈 Do not allow editing
                                                isEditableRole={isLoggedInUserAdmin} />
                                            <Password
