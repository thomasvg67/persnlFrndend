...
const Fund = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${VITE_BASE_URL}/api/fund?type=${type}`, {
    ...
    try {
        await axios.post(`${VITE_BASE_URL}/api/fund`, { name,type }, {
            headers: { Authorization: `Bearer ${token}` },
        });
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/fund/${selectedList._id}`, { name }, {
            headers: { Authorization: `Bearer ${token}` },
        });
    ...
    setOperationLoading(true);
    try {
        await axios.post(`${VITE_BASE_URL}/api/fund/${selectedList._id}/visions`, {
            title: taskTitle,
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/fund/${selectedList._id}/visions/${selectedTask._id}`, {
            title: taskTitle,
            description: taskDesc,
        }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    try {
        if (deleteType === 'list' && deleteList) {
            await axios.delete(`${VITE_BASE_URL}/api/fund/${deleteList._id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            toast.success('Mission deleted successfully');
        } else if (deleteType === 'task' && deleteTask && selectedList) {
            await axios.delete(`${VITE_BASE_URL}/api/fund/${selectedList._id}/visions/${deleteTask._id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            toast.success('Vision deleted successfully');
        } else if (deleteType === 'clear' && clearTasksList) {
            await axios.put(`${VITE_BASE_URL}/api/fund/${clearTasksList._id}/clear-visions`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/fund/${listId}/clear-visions`, {}, {
            headers: { Authorization: `Bearer ${token}` },
        });
                ...
                <div id="content" className="main-content">
                    <div className="layout-px-spacing">
                        <div className="action-btn layout-top-spacing mb-5">
                            <button id="add-list" className="btn btn-dark" onClick={onAddList}>Add Fund</button>
                        </div>
                            ...
                            <div className="modal-dialog modal-dialog-centered" role="document">
                                <div className="modal-content">
                                    <div className="modal-body">
                                        <div className="compose-box">
                                            <div className="compose-content" id="addListModalTitle">
                                                <h5 className="add-list-title" style={{ display: selectedList ? 'none' : 'block' }}>Add Fund</h5>
                                                <h5 className="edit-list-title" style={{ display: selectedList ? 'block' : 'none' }}>Edit Fund</h5>
                                                <form action="javascript:void(0);" onSubmit={(e) => e.preventDefault()}>
                                                    <div className="row">
                                                        <div className="col-md-12">
                                    ...
                                    <div className="modal-footer">
                                        ...
                                        {!selectedList && (<button className="btn add-list" onClick={handleAddList}>Add Fund</button>)}
                                ...
                                <div className="task-list-section">
                                    ...
                                    {loading ? (<Loader />) :
                                        lists && lists.length > 0 ? (lists.map((list) => (
                                            ...
                                            >
                                                <h5 className="text-muted">No Fund saved</h5>
                                            </div>
                                        )}
                                </div>
                            </div>
