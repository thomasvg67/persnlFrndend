...
const BussIdea = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${VITE_BASE_URL}/api/business`, {
                headers: {
    ...
    try {
        await axios.post(`${VITE_BASE_URL}/api/business`, { name }, {
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/business/${selectedList._id}`, { name }, {
            headers: { Authorization: `Bearer ${token}` }
        });
        setName('');
    ...
    setOperationLoading(true);
    try {
        await axios.post(`${VITE_BASE_URL}/api/business/${selectedList._id}/ideas`, {
            title: taskTitle,
        ...
        toast.error(err.response?.data?.message || 'Failed to add idea');
    } finally {
        setOperationLoading(false);
    }
};
   ...
   const handleEditTask = async () => {
    ...
    if (!selectedList?._id || !selectedTask?._id) {
        console.error("Edit task failed:", { selectedList, selectedTask });
    ...
    setOperationLoading(true);
    try {
        await axios.put(`${VITE_BASE_URL}/api/business/${selectedList._id}/ideas/${selectedTask._id}`, {
            title: taskTitle,
            description: taskDesc,
        }, {
            headers: { Authorization: `Bearer ${token}` }
    ...
    try {
        if (deleteType === 'list' && deleteList) {
            await axios.delete(`${VITE_BASE_URL}/api/business/${deleteList._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            toast.success('Business deleted successfully');
        } else if (deleteType === 'task' && deleteTask && selectedList) {
            await axios.delete(`${VITE_BASE_URL}/api/business/${selectedList._id}/ideas/${deleteTask._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            toast.success('Idea deleted successfully');
        } else if (deleteType === 'clear' && clearTasksList) {
            await axios.put(`${VITE_BASE_URL}/api/business/${clearTasksList._id}/clear-ideas`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleClearAllTasks = async (listId) => {
        ...
        try {
            await axios.put(
                `${VITE_BASE_URL}/api/business/${listId}/clear-ideas`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
            ...
            alert(err.response?.data?.message || 'Failed to clear tasks');
        } finally {
            setOperationLoading(false);
        }
    };
                ...
                <div id="content" className="main-content">
                    <div className="layout-px-spacing">
                        <div className="action-btn layout-top-spacing mb-5">
                            <button id="add-list" className="btn btn-dark" onClick={onAddList}>Add Bussiness</button>
                        </div>
                            ...
                            <div className="modal-dialog modal-dialog-centered modal-xl" role="document">
                                <div className="modal-content">
                                    <div className="modal-body">
                                        <div className="compose-box">
                                            <div className="compose-content" id="addTaskModalTitle">
                                                <h5 className="add-task-title" style={{ display: selectedTask ? 'none' : 'block' }}>Add Idea</h5>
                                                <h5 className="edit-task-title" style={{ display: selectedTask ? 'block' : 'none' }}>Edit Idea</h5>
                                    ...
                                    <div className="modal-footer">
                                        ...
                                        {!selectedTask && (<button data-btnfn="addTask" className="btn add-tsk" onClick={handleAddTask}>Add Idea</button>)}
                            ...
                            <div className="modal-dialog modal-dialog-centered" role="document">
                                <div className="modal-content">
                                    <div className="modal-body">
                                        <div className="compose-box">
                                            <div className="compose-content" id="addListModalTitle">
                                                <h5 className="add-list-title" style={{ display: selectedList ? 'none' : 'block' }}>Add Bussiness</h5>
                                                <h5 className="edit-list-title" style={{ display: selectedList ? 'block' : 'none' }}>Edit Bussiness</h5>
                                                <form action="javascript:void(0);" onSubmit={(e) => e.preventDefault()}>
                                                    <div className="row">
                                                        <div className="col-md-12">
                                                            <div className="list-title">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="feather feather-list"><line x1={8} y1={6} x2={21} y2={6} /><line x1={8} y1={12} x2={21} y2={12} /><line x1={8} y1={18} x2={21} y2={18} /><line x1={3} y1={6} x2={3} y2={6} /><line x1={3} y1={12} x2={3} y2={12} /><line x1={3} y1={18} x2={3} y2={18} /></svg>
                                                                <input id="s-list-name" type="text" placeholder="Bussiness Name" className="form-control" name="task" value={name} onChange={(e) => setName(e.target.value)} required />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </form>
                                    ...
                                    <div className="modal-footer">
                                        ...
                                        {!selectedList && (<button className="btn add-list" onClick={handleAddList}>Add Bussiness</button>)}
                            ...
                            <div className="modal-dialog modal-dialog-centered" role="document">
                                <div className="modal-content" id="deleteConformationLabel">
                                    <div className="modal-header">
                                        ...
                                        <h5 className="modal-title" id="exampleModalLabel">Delete the Idea?</h5>
                                        <button type="button" className="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">×</span>
                                        </button>
                                    </div>
                                    <div className="modal-body">
                                        <p className=""> {deleteType === 'list' && "If you delete the Bussiness it will be gone forever. Are you sure you want to proceed?"}</p>
                                        <p className=""> {deleteType === 'task' && "If you delete the Idea it will be gone forever. Are you sure you want to proceed?"}</p>
                                        <p className="">{deleteType === 'clear' && "If you clear all the Bussiness it will be gone forever. Are you sure you want to proceed?"}</p>
                                    </div>
                                    <div className="modal-footer">
